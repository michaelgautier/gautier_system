...
void gautier::system::rss::gautier_rss_requestor::request_feeds(
				...
				Poco::HTTPClientSession http_session = Poco::HTTPClientSession();

				for(auto rss_location : feed_parameters) {
					
					Poco::HTTPRequest http_request(request_method, rss_location.feed_url);
					
					http_session.sendRequest(http_request);
					
					Poco::HTTPResponse http_response;
					
					std::istream http_response_stream;
					
					http_response_stream = http_session.receiveResponse(http_reponse);
